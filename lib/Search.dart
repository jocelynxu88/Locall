...
class DataSearch extends SearchDelegate<String> {
  ...
  List<JobData> _jobs = [
    ...
    for(int i = 0;i < _jobs.length;i++){
        bool found = false;
        if(_jobs[i].description.toLowerCase().contains(query.toLowerCase())) found = true;
        for(int j = 0;j < _jobs[i].tags.length && !found;j++){
            if(_jobs[i].tags[j].toLowerCase().contains(query.toLowerCase()) || query.toLowerCase().contains(_jobs[i].tags[j].toLowerCase())){
                found = true;
            }
        }
        if(found) filteredJobs.add(_jobs[i]);
    }
    ...
    return ListView.builder(
        ...
        itemBuilder: (context, i){
            ...
            return _buildJob(filteredJobs[i], context);
        }
    );
  }

    Widget _buildJob(JobData job, BuildContext context){
